...
filename = "data_modified_new_numpy_from_net.csv"
...
it=0
while(1):
	random_sample=random.sample(range(590), 8)
	clusters=[[],[],[],[],[],[],[],[]]
	centroid=[];
	centroid_new=[];
...
#centroid=np.ones((8,8266))
	...
	for j in range(590):              #change
		distance=[];
		for i in range(0,8):
			distance.append(round(((centroid[i].dot(data[j]))/(norm(centroid[i])*norm(data[j]))),4));
...
while(it>=0):
	...
	np.array(clusters).dump("clusters_k_means.txt")
