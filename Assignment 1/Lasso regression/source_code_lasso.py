...
while (error_diff>=0.0000001):                                 #error difference threshold set to 10^-7
  ...
  weights[1:]=weights[1:]-(l_r*(np.transpose(feature)).dot(feature.dot(weights)-labels)/1000-(2*l_r*lambda1/1000)*np.ones((order+1,1)))[1:] #weights updated
  weights[0]=weights[0]-(l_r*(np.transpose(feature)).dot(feature.dot(weights)-labels)/1000)[0]
