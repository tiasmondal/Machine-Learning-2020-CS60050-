...
while (error_diff>=0.0000001):                                 #error difference threshold set to 10^-7
  error1=np.sum((feature.dot(weights)-labels)**2)/(2*1000)     #Previous error
  weights=weights-l_r*(np.transpose(feature)).dot(feature.dot(weights)-labels)/1000   #weights update
  error=np.sum((feature.dot(weights)-labels)**2)/(2*1000)    #Current errpr
...
plt.title("Fitted curve for order "+ str(order)+" on test data",color="white")
plt.legend()
plt.show()
fig=plt.figure()
...
plt.ylabel("Predicted labels",color="white")
plt.legend()
plt.title("Fitted curve for order "+str(order)+" on training data data",color="white")
plt.show()
